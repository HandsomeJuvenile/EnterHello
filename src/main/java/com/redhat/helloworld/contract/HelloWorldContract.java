...
public class HelloWorldContract extends Contract implements HelloWorldInterface {
    ...
    public RemoteCall<TransactionReceipt> send(String reciver, int amount) {
        Function function = new Function("send", Arrays.<Type>asList(new Utf8String(reciver),new Uint256(amount)),
                Arrays.<TypeReference<?>>asList());
        return executeRemoteCallTransaction(function);
    }
    ...
    public RemoteCall<TransactionReceipt> mint(String reciver, int amount) {
        Function function = new Function("mint", Arrays.<Type>asList(new Utf8String(reciver),new Uint256(amount)),
                Arrays.<TypeReference<?>>asList());
        return executeRemoteCallTransaction(function);
    }
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            HelloWorldContract registerContract = new HelloWorldContract(Consts.REGISTER_ADDR,web3j,credentials,Consts.GAS_PRICE,Consts.GAS_LIMIT);
            registerContract.mint("0x3e739945a4D5c649a082C410d415FFaDBAeBb810",456);
        } catch (IOException e) {
            e.printStackTrace();
